...
glm = c(0, 1, 2, 3)
ave = c(0.000069,0.0033,0.201, 0.794)
sd = c(0.000008, 0.00037, 0.0226, 0.0894)
...
# make your own log10
dfPlot <- data %>% 
  mutate(ave = -log10(ave))

pl = ggplot(data = data, aes(x = glm, y = ave, fill = I('gray40'), color = I('black'))) +
  geom_bar(stat = "identity", alpha = .3, width=0.5) +
  geom_errorbar(aes(ymax = ave + sd, ymin = ave - sd), width=0.05,  alpha=1)

#, expand = c(0, 0), breaks = c(seq.int(0,1, by = 0.2)), limits = c(-0.05,1)
plt = pl + 
  scale_y_continuous(expand = c(0, 0), breaks = c(seq.int(0,1, by = 0.2)), limits = c(-0.05,1)) +
  # scale_y_log10(
  #   breaks = scales::trans_breaks("log10", function(x) 10^x),
  #   labels = scales::trans_format("log10", scales::math_format(10^.x)), limits = c(0.00006, 1)) +
  scale_x_discrete(labels=c("GLM I", "GLM II", "GLM III", "GLM IV")) +
  theme_bw() +
