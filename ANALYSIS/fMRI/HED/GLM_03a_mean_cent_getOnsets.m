...
%% FOR SPM
        ...
        onsets.reward      = ONSETS.sniffSignalOnset(strcmp ('chocolate', CONDITIONS));
        onsets.neutral     = ONSETS.sniffSignalOnset(strcmp ('neutral', CONDITIONS));
        onsets.control     = ONSETS.sniffSignalOnset(strcmp ('empty', CONDITIONS));

        
        %get durations
        durations.reward    = DURATIONS.trialstart(strcmp ('chocolate', CONDITIONS));
        durations.neutral   = DURATIONS.trialstart(strcmp ('neutral', CONDITIONS));
        durations.control   = DURATIONS.trialstart(strcmp ('empty', CONDITIONS));
       
        
        %mod for liking 
        modulators.reward.lik  = BEHAVIOR.liking (strcmp ('chocolate', CONDITIONS));
        modulators.neutral.lik = BEHAVIOR.liking (strcmp ('neutral', CONDITIONS));
        modulators.control.lik = BEHAVIOR.liking (strcmp ('empty', CONDITIONS));
        
        %mean_centering mod
        cent_reward  = mean(modulators.reward.lik);
        cent_neutral = mean(modulators.neutral.lik);
        cent_control = mean(modulators.control.lik);
        
        for j = 1:length(modulators. reward.lik)
             modulators.reward.lik(j)  = modulators.reward.lik(j) - cent_reward;
             modulators.neutral.lik(j) = modulators.neutral.lik(j) - cent_neutral;
             modulators.control.lik(j) = modulators.control.lik(j) - cent_control;
        end
%         %mod for intensity
