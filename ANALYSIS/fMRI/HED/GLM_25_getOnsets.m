function GLM_25_getOnsets()

% intended for REWOD HED
% like 22 but durations = 0
...
for  i=1:length(subj)
    ...
    durations.start    = zeros (length(onsets.start),1);
    ...
    %get durations
    durations.odor          = zeros (length(onsets.smell),1);
    ...
    %mean_centering mod
    %RPE
    cent_int  = mean(modulators.odor.RPE);
    
    for j = 1:length(modulators.odor.RPE)
        modulators.odor.RPE(j)  = modulators.odor.RPE(j) - cent_int;
    end
    
    
    %liking
    cent_int  = mean(modulators.odor.lik);
    
    for j = 1:length(modulators.odor.lik)
        modulators.odor.lik(j)  = modulators.odor.lik(j) - cent_int;
    end
