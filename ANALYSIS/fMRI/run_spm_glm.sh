...
do
	# prep for each session's data
        ...
        qsub -o /home/REWOD/ClusterOutput3 -j oe -l walltime=4:40:00,pmem=4GB -M eva.pool@unige.ch -m n -l nodes=1  -q queue1 -N ${GLM}_s${subj}_${task} -F "${subj} ${codeDir} ${matlab_script}" ${matlabSubmit}
