function GLM_between_control_ndLevel()
...
covdir   = fullfile (homedir, 'DERIVATIVES/GLM/ForPaper', task, name_ana, 'group_covariates'); % director with the extracted second level covariates
...
for n = 1:length(contrastNames)
    ...
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.weights  = [1]; % in the covariate the second colon is the one of interest
    matlabbatch{3}.spm.stats.con.consess{1}.tcon.sessrep  = 'none';
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.name     = ['Neg ' contrastX(1:end)];
    matlabbatch{3}.spm.stats.con.consess{2}.tcon.weights  = [-1]; % in the covariate the second colon is the one of interest
