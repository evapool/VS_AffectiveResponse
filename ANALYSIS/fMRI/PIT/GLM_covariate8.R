...
for(i in s) {
  ...
  covpath = paste(home_path, '/DERIVATIVES/GLM/',taskDIR, '/' , GLM , '/', subj , '/timing', sep = '')
  cov_minus <- read.delim(file.path(covpath, paste(GLM, '_task-PIT_ndl_CSm_eff.txt', sep = '')), header = F, sep ='\t') # read in dataset
  cov_plus <- read.delim(file.path(covpath, paste(GLM, '_task-PIT_ndl_CSp_eff.txt', sep = '')), header = F, sep ='\t') # read in dataset
  
  CSp_CSm = cov_plus - cov_minus
  df[l,2] = mean(CSp_CSm[,1])
}
