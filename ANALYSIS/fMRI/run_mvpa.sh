...
# Loop over subjects
 for subjectID in 01 #02 03 04 05 06 07 09 10 11 12 13 14 15 16 17 18 20 21 22 23 24 25 26 # 
    do
			qsub -o ${home}/REWOD/ClusterOutput -j oe -l walltime=10:40:00,pmem=16GB -M david.munoz@etu.unige.ch -m n -l nodes=1 -q queue1 -N ${model}_s${subjectID}_${taskID} -F "${subjectID} ${taskID} ${model} ${codeDir} ${MVPA_script}" ${pythonSubmit}
            ...
            #qsub -o ${home}/REWOD/ClusterOutput -j oe -l walltime=2:40:00,pmem=GB -M david.munoz@etu.unige.ch -m n -l nodes=1  -q queue1 -N grid_search_${subjectID} -F "${subjectID} ${taskID} ${model} ${codeDir} ${MVPA_script}" ${pythonSubmit}
