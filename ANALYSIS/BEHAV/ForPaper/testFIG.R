...
BMS_PIT_path <- file.path(home_path, '/DERIVATIVES/GLM/ForPaper/PIT/BMS/') 
BMS_HED_path <- file.path(home_path, '/DERIVATIVES/GLM/ForPaper/hedonic/BMS/') 
...
#diffI = select(HED.int.index, c(ID, delta_int))
#diffI$ID <- as.numeric(as.character(diffI$ID))
#write.table(diffI, (file.path(covariateHED_path, "Odor_NoOdor.txt")), row.names = F, sep="\t")
...
PIT_BMS <- read.csv(paste(BMS_PIT_path, 'PIT_BMS.csv', sep = ''))
...
HED_BMS <- read.csv(paste(BMS_HED_path, 'hedonic_BMS.csv', sep = ''))
