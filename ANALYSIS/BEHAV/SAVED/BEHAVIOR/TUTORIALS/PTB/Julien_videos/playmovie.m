...
%% display first frame, start sound
if ~h.usePsychAudio
    resume(h.audObj);
else
...
while h.curFrame <= h.nFrames,
    ...
    if GetSecs - h.actualFrameTime(1) >= h.frameTime(h.curFrame)
        ...
        if ismember(h.curFrame,h.startShort - h.startFrame + 1),
            sendTTLsJD(h.TTL.startShort,[h.curFrame h.curFrame + h.startFrame - 1],h);
        end
        if ismember(h.curFrame,h.endShort - h.startFrame + 1),
            sendTTLsJD(h.TTL.endShort,[h.curFrame h.curFrame + h.startFrame - 1],h);
        end
        if ismember(h.curFrame,h.isOld - h.startFrame + 1),
            sendTTLsJD(h.TTL.isOld,[h.curFrame h.curFrame + h.startFrame - 1],h);
        end
        if ismember(h.curFrame,h.isNew - h.startFrame + 1),
            sendTTLsJD(h.TTL.isNew,[h.curFrame h.curFrame + h.startFrame - 1],h);
        end
        ...
        if mod(h.curFrame,h.frameRate)==300, % 3min
            inputemu({...
