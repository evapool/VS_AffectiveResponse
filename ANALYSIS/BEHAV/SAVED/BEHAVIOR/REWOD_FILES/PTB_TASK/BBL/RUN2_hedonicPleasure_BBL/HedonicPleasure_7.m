...
% odor rating: hedonic pleasure % modified on the 12.06.2015 by Eva
...
elseif var.day == 2
    var.repetitions = 6;% determine the amount of time a each odor will be repeteated
    var.lengthBlock = 3; %define how many
end
...
var.trigTrialStart = var.trigTrialStart (randomIndex);
    ...
    for i = 1:length(var.trig);
        ...
        for mb = 1:var.lengthBlock; % MiniBlock: three repetion of the same odor
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%% Pleasantness %%%%%%%%%%%%%%%%%%%%%
            l1 = GetSecs;
            data.likingOnset(nTrial,1) = GetSecs -var.time_MRI;
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%% Intensity %%%%%%%%%%%%%%%%%%%%%%%%
            I1 = GetSecs;
            data.intensityOnset(nTrial,1) = GetSecs -var.time_MRI;
