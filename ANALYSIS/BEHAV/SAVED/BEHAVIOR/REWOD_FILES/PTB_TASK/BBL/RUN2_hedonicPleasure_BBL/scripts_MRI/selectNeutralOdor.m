 function [neutralOdor,fileDoesNotExist] = selectNeutralOdor (referenceOdor,empthyOdor,var) %% modified on the 4.june 2015
    ...
    %%% when the van open (var.trig)
        ...
        differentialIntensity = matrixToSelect (:,3);
        
        %%% select the neutral odor based on the index of interest
        
        neutralOdor = matrixToSelect (1,1);
        if differentialIntensity (1) >= 45 % this thrashold has been established by me.. maybe there is better way to do it
            neutralOdor = matrixToSelect (2,1);
            if differentialIntensity (2) >= differentialIntensity (1) % if the second most neutral odor is even less than the first one, then the first is kept
                neutralOdor = matrixToSelect (1,1);
            end
        end
