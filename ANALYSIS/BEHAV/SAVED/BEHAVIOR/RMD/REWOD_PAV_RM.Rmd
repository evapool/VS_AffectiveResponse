...
#shorter RT than 100ms and longer than 3sd+mean
...
```{r boxplots RT distribution, fig.align = "center"}
# reaction time by conditions #(baseline non included)
REWOD_PAV.RT <- REWOD_PAV
REWOD_PAV.RT$condition <- droplevels(REWOD_PAV.RT$condition, exclude = "Baseline")
boxplot(REWOD_PAV.RT$RT ~ REWOD_PAV.RT$condition, las = 1)
...
```{r boxplots overall effect rt, fig.align = "center"}
...
```{r boxplots overall effect ratings, fig.align = "center"}
# condition X ratings
...
#filter(ba, condition == 'CSminus')#CSminus Acc = 0.9830247
#filter(ba, condition == 'CSplus')#CSplus Acc = 0.9783951
#summarise(bsacc, min(accuracy)) # min acc is 0.8703704 (sub 17)
```