...
#SETUP
task = 'hedonic'
con_name1 = 'Reward1_NoReward1_9'
con_name2 = 'CSp_CSm'



con1 = 'Reward1_NoReward1'
con2 = 'CSp-CSm'
...
LIK_Lik <- read.delim(file.path(analysis_path, 'GLM-10', 'group_covariates', paste(con1,'_', mod1, '_meancent.txt',sep="")), header = T, sep ='\t') # read in dataset
...
A6 <- ggplotRegression(lm(R_NoR_df[[7]]~lik)) + rremove("x.title")



ggarrange(A1,A2,A3,A4,A5,A6,
                     labels = c(    "cmOFC_cluster_betas"   ,  "cm_OFC_sphere_betas"  ,   "p_core_L_cluster_betas"  ,"p_shell_R_cluster_betas",
                                   "pcore_LEFT_betas"   ,     "pshell_RIGHT_betas" ),
                     ncol = 2, nrow = 3,
                     vjust=3, hjust=0) 


#cm_OFC_sphere_betas = anatomical sphere
...
# check = lmRob(R_NoR_df$cm_OFC_sphere_betas~R_NoR_df$lik)
...
P1 <- ggplot(R_NoR_df, aes(lik, cm_OFC_sphere_betas)) + #A2
  ...
  #scale_x_continuous(name="Hedonic experience", expand = c(0, 0), limits=c(-2.5, 1.5)) +
...
figure1 <- annotate_figure(P1,
                           top = text_grob(paste("Robust: anatomicaly defined left cm_OFC ~ lik: p", round(p_rob,5)), color = "black", face = "bold", size = 14))
...
P2 <- ggplot(R_NoR_df, aes(lik, cm_OFC_sphere_betas)) + #A2
  geom_point() +
...
P3 <- ggplot(CSp_df, aes(eff, cm_OFC_sphere_betas)) + #A2
...
P4 <- ggplot(CSp_df, aes(eff, cm_OFC_sphere_betas)) + #A2
  geom_point() +
...
figure1  #rob cm_OFC ~ lik 
figure2   #reg cm_OFC ~ lik
figure3  #rob cm_OFC ~ eff
figure4  #reg cm_OFC~ eff
...
#well the CI are still in 0.. even for robust
