...
INT_Lik <- read.delim(file.path(analysis_path, 'GLM-04', 'group_covariates', paste(con1,'_', mod3, '_meancent.txt',sep="")), header = T, sep ='\t') # read in dataset
...
R_NoR_df  = merge(R_NoR_df , INT_Lik, by.x = "ID", by.y = "subj", all.x = TRUE)
...
# int = R_NoR_df$int
# int = zscore(int)
# R_NoR_df$int = zscore(int)
# A1 <- ggplotRegression(lm(R_NoR_df[[2]]~int)) + rremove("x.title")
# A2 <- ggplotRegression(lm(R_NoR_df[[3]]~int)) + rremove("x.title")
# A3 <- ggplotRegression(lm(R_NoR_df[[4]]~int)) + rremove("x.title")
# A4 <- ggplotRegression(lm(R_NoR_df[[5]]~int)) + rremove("x.title")
# A5 <- ggplotRegression(lm(R_NoR_df[[6]]~int)) + rremove("x.title")
# 
...
grob0 <- grobTree(textGrob("WLS:", x=0.7,  y=0.15, hjust=0,
                           gp=gpar(col="black", fontsize=9, fontface="italic")))

grob1 <- grobTree(textGrob(bquote(paste(r^2," = ",.(signif(rsq,2)))), x=0.7,  y=0.1, hjust=0,
                           gp=gpar(col="black", fontsize=9, fontface="italic")))

grob2 <- grobTree(textGrob(bquote(paste("p = ",.(signif(p_rob,2)))), x=0.7,  y=0.05, hjust=0,
                           gp=gpar(col="black", fontsize=9, fontface="italic")))
...
grob0 <- grobTree(textGrob("LS:", x=0.7,  y=0.15, hjust=0,
                           gp=gpar(col="black", fontsize=9, fontface="italic")))

grob1 <- grobTree(textGrob(bquote(paste(r^2," = ",.(signif(rsq,2)))), x=0.7,  y=0.1, hjust=0,
                           gp=gpar(col="black", fontsize=9, fontface="italic")))

grob2 <- grobTree(textGrob(bquote(paste("p = ",.(signif(p_par,2)))), x=0.7,  y=0.05, hjust=0,
                           gp=gpar(col="black", fontsize=9, fontface="italic")))
...
par = summary(lm(R_NoR_df$cm_OFC_sphere_betas~R_NoR_df$int))
p_par = par$coefficients[8]

rob = rlm(R_NoR_df$cm_OFC_sphere_betas~R_NoR_df$int)
rsq = summary(lmRob(R_NoR_df$cm_OFC_sphere_betas~R_NoR_df$int))$r.squared
...
R_NoR_df$OFC2_sphere_weights = weights
R_NoR_df$OFC2_sphere_weights = I(R_NoR_df$OFC2_sphere_weights)
...
P9 <- ggplot(R_NoR_df, aes(int, cm_OFC_sphere_betas)) + #A2
  geom_point(aes(alpha=R_NoR_df$OFC2_sphere_weights)) +
  geom_smooth(method = "rlm",  col = "orange") +
  #geom_smooth(lm(R_NoR_df$cm_OFC_sphere_betas~R_NoR_df$int, w = 1/R_NoR_df$cm_OFC_sphere_betas^2)
  scale_x_continuous(name="Intensity Index", expand = c(0, 0), limits=c(-2.5, 2.5), breaks=c(seq.int(-2.5,2.5, by = 1))) +
  #scale_x_continuous(name="Pleasantness Index", expand = c(0, 0), limits=c(-2, 2)) +
  scale_y_continuous(expression(paste(beta, "  Reward > No Reward")), expand = c(0, 0), limits=c(-0.8, 1.2), breaks=c(seq.int(-0.8,1.2, by = 0.5))) +
  theme(plot.subtitle = element_text(size = 8, vjust = -90, hjust =1), panel.grid.major = element_blank(), legend.position = "none", panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), margin = NULL, aspect.ratio=1)  +
...
#anatomicaly defined left cm_OFC ~ int
...
CI =CI.Rsq(rsq, n, k, level = 0.9)
stat_9 = paste("r² = ", signif(rsq,3),  ", 90% CI [", signif(CI$LCL,3), ",", signif(CI$UCL,3), "]", ", p = ", round(p_rob, 5))

# PLOT cmOFC int norm -----------------------------------------------------------

#  _R2_ 
rsq =summary(lm(R_NoR_df$cm_OFC_sphere_betas~int))$r.squared
...
P10 <- ggplot(R_NoR_df, aes(int, cm_OFC_sphere_betas)) + #A2
  geom_point() +
...
CI =CI.Rsq(rsq, n, k, level = 0.9)
stat_10 = paste("r²   = ", signif(rsq,3),  ", 90% CI [", signif(CI$LCL,3), ",", signif(CI$UCL,3), "]", ", p = ", round(p_par, 5))
...
figure11 <- ggarrange(P5,P6,P7,P8,P9,P10,
