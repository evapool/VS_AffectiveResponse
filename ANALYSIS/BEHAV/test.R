...
# open dataset 
BETAS_R_NoR <- read.delim(file.path(analysis_path, 'ROI', paste('extracted_betas_',con_name1,'_via_', con_name2, '.txt',sep="")), header = T, sep ='\t') # read in dataset
...
figure1 <- ggarrange(A1,A2,A3,
                     labels = c(  "BLA_lik_L_betas", "cluster_vNAcc_R_betas"),
                     ncol = 2, nrow = 2,
                     vjust=3, hjust=0) 
