...
mdldir   = fullfile(homedir, '/DERIVATIVES/GLM/', 'hedonic');
...
for i = 1:length(param.task)
    ...
    param.modul{i} = {'none',...%1
        'ONS.modulators.odor.reward.chAbs',... %2
        'ONS.modulators.odor.neutral.chAbs',... %3
        'ONS.modulators.odor.control.chAbs',... %4
        'none',... %5
        'none'}; %6
...
%% function section
    ...
    function [] = doContrasts(subjoutdir, param, SPM)
        ...
        conditionName{ncondition} = strcat(task,'constant'); %ajust for the last condition
